...
def remove_outliers_iqr(df, columns):
    """
    Remove outliers from a DataFrame using the IQR method for specified columns.
...
def plot_confidence_intervals_by_utility(csv_file, confidence=0.95, output_dir='plots/confidence_intervals'):
    ...
    rename utility types, remove outliers, and plot them with each numerical column in its own subplot.
...
def plot_cdf_by_utility(csv_file, t_gpu_min=None, t_gpu_max=None, output_dir='plots/cdf'):
    """
    Read a CSV file, group data by utility types, remove outliers, and plot the Cumulative Distribution Function (CDF)
...
def plot_cdf_comparison_all_intervals(csv_file, t_gpu_ranges, selected_columns, utility_types, output_dir='plots/cdf_comparison'):
    """
    ...
    pass  # Placeholder for optional implementation
...
if __name__ == "__main__":
    ...
    csv_file_path = '50J_50N_NFD_HN_NDJ_BW_test_results'  # Example: 'final_allocations'

    # Read the CSV to compute percentiles
    try:
        df_main = pd.read_csv(csv_file_path + '.csv')
    except FileNotFoundError:
        print(f"Error: File '{csv_file_path}.csv' not found.")
        exit(1)
    except pd.errors.EmptyDataError:
        print("Error: CSV file is empty.")
        exit(1)
    ...
    if t_gpu_data.empty:
        print("Error: 't_gpu' column contains only NaN values.")
        exit(1)
    ...
    print(f"Computed t_gpu Percentiles:")
    ...
    print(f"Defined t_gpu_ranges based on percentiles: {t_gpu_ranges}")
