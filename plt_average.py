...
import matplotlib.ticker as mtick  # Added for y-axis formatting
...
def customize_legend(ax, text_size, title_text_size, frameon=False):
    """
    ...
    - frameon: Boolean indicating whether to display a frame around the legend
    """
    lines, labels = ax.get_legend_handles_labels()
    num_labels = len(labels)
    if num_labels == 0:
        return  # No legend to customize
...
for filename in file_list:
    ...
    if match:
        ...
        if utility in SELECTED_UTILITIES:
            ...
            required_columns = ['complete_time', 'submit_time', 'duration', 'num_pod', 'num_gpu', 'final_node_allocation', 'complete_time']
            ...
            numeric_columns = ['complete_time', 'submit_time', 'duration', 'num_pod', 'num_gpu', 'complete_time']
...
# -------------------------
# **Filtering GPUs to Start from 0.5 (num_gpu >= 50)**
# -------------------------
# Since num_gpu is an integer, filtering to start from 0.5 (which corresponds to num_gpu >= 50)
all_data = all_data[all_data['num_gpu'] >= 50]
...
for allocation in allocation_types:
    ...
    # -------------------------
    plot_type = 'metric_vs_num_pod_average'
    ...
    # Format y-axis to show only single decimals
    ax = plt.gca()
    ax.yaxis.set_major_formatter(mtick.FormatStrFormatter('%.1f'))

    # Customize the legend with two lines
    customize_legend(ax, TEXT_SIZE, TEXT_SIZE, frameon=True)

    plt.tight_layout()

    # Save the plot in PNG format
    avg_pod_plot_path_png = os.path.join(PLOTS_DIRECTORY, f'metric_vs_num_pod_average_{allocation}.png')
    plt.savefig(avg_pod_plot_path_png)

    # Save the plot in SVG format
    avg_pod_plot_path_svg = os.path.join(PLOTS_DIRECTORY, f'metric_vs_num_pod_average_{allocation}.svg')
    plt.savefig(avg_pod_plot_path_svg)

    plt.close()

    # -------------------------
    # Plot 2: Average Metric vs. Number of GPUs (Divided by 100)
    # -------------------------
    plot_type = 'metric_vs_num_gpu_average'
    ...
    # Create a new column with num_gpu divided by 100
    avg_gpu_metric['num_gpu_div100'] = avg_gpu_metric['num_gpu'] / 100

    # Line plot of average metric vs. num_gpu_div100, colored by utility
    plt.figure(figsize=(PLOT_WIDTH, PLOT_HEIGHT))
    sns.lineplot(data=avg_gpu_metric, x='num_gpu_div100', y='metric', hue='utility', marker='o')

    # Set plot labels
    plt.xlabel('Number of GPUs')  # Updated label to reflect division
    plt.ylabel('Normalized JCT')
    ...
    # Save the plot in PNG format
    avg_gpu_plot_path_png = os.path.join(PLOTS_DIRECTORY, f'metric_vs_num_gpu_average_{allocation}.png')
    plt.savefig(avg_gpu_plot_path_png)

    # Save the plot in SVG format
    avg_gpu_plot_path_svg = os.path.join(PLOTS_DIRECTORY, f'metric_vs_num_gpu_average_{allocation}.svg')
    plt.savefig(avg_gpu_plot_path_svg)
    ...
    # -------------------------
    plot_type = 'nodes_vs_num_pod_average'
    ...
    if avg_pod_nodes_filtered.empty:
        ...
        # Set plot labels
        plt.xlabel('Number of Workers')
        plt.ylabel('Average Gang Schedule')
        ...
        # Save the plot in PNG format
        avg_pod_nodes_plot_path_png = os.path.join(PLOTS_DIRECTORY, f'nodes_vs_num_pod_average_{allocation}.png')
        plt.savefig(avg_pod_nodes_plot_path_png)

        # Save the plot in SVG format
        avg_pod_nodes_plot_path_svg = os.path.join(PLOTS_DIRECTORY, f'nodes_vs_num_pod_average_{allocation}.svg')
        plt.savefig(avg_pod_nodes_plot_path_svg)

        plt.close()

    # -------------------------
    # Plot 4: Average Number of Nodes vs. Number of GPUs (Divided by 100)
    # -------------------------
    plot_type = 'nodes_vs_num_gpu_average'
    ...
    # Create a new column with num_gpu divided by 100
    avg_gpu_nodes['num_gpu_div100'] = avg_gpu_nodes['num_gpu'] / 100
    ...
    if avg_gpu_nodes_filtered.empty:
        ...
        # Line plot of average number of nodes vs. num_gpu_div100, colored by utility
        plt.figure(figsize=(PLOT_WIDTH, PLOT_HEIGHT))
        sns.lineplot(data=avg_gpu_nodes_filtered, x='num_gpu_div100', y='num_nodes', hue='utility', marker='o')
        ...
        # Save the plot in PNG format
        avg_gpu_nodes_plot_path_png = os.path.join(PLOTS_DIRECTORY, f'nodes_vs_num_gpu_average_{allocation}.png')
        plt.savefig(avg_gpu_nodes_plot_path_png)

        # Save the plot in SVG format
        avg_gpu_nodes_plot_path_svg = os.path.join(PLOTS_DIRECTORY, f'nodes_vs_num_gpu_average_{allocation}.svg')
        plt.savefig(avg_gpu_nodes_plot_path_svg)
